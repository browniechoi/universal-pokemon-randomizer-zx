...
public class BDSPRomHandler extends AbstractSwitchRomHandler {
    ...
    private static boolean detectSwitchGameInner(String filePath) {
        // No other Pokemon games on Switch are in Unity (currently), so it's a signal that this is BDSP.
        String bootConfigPath = filePath + File.separator + "romfs" + File.separator + "boot.config";
        File bootConfigFile = new File(bootConfigPath);
        return (bootConfigFile.exists() && bootConfigFile.isFile());
    }
